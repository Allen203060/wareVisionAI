...
class ProductListCreateAPIView(generics.ListCreateAPIView):
    """
    API view to retrieve a list of all products or create a new product.
    Handles GET and POST requests to /api/products/.
    """
...
class ProductDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    """
    API view to retrieve, update, or delete a single product by its ID.
    Handles GET, PUT, PATCH, and DELETE requests to /api/products/<int:pk>/.
    """
    queryset = Product.objects.all()
    serializer_class = ProductSerializer
...
class ProposeActionAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # The LLM might return a list of IDs if the query is ambiguous.
...
class ExecuteActionAPIView(APIView):
    """
    Receives a confirmed action from the frontend (from the LLM workflow) and executes it on the database.
    """
    parser_classes = [JSONParser]
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == "UPDATE":
                ...
                # We need to map the fields from the LLM to the model fields
                serializer_data = {
                    'product_name': update_data.get('product_name', product_to_modify.product_name),
                    'price': update_data.get('price', product_to_modify.price),
                    'expiry_date': update_data.get('expiry_date', product_to_modify.expiry_date)
                }
                serializer = ProductSerializer(product_to_modify, data=serializer_data, partial=True)
